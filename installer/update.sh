...
check_existing_installation() {
    print_header "Vorhandene Installation prüfen"

    if [[ ! -x "${BIN_DIR}/pdf-tool" ]]; then
        print_warn "Kein vorhandener Launcher unter ${BIN_DIR}/pdf-tool gefunden."
        print_warn "Falls PDFTool noch nicht installiert ist, zuerst ./installer/install.sh ausführen."
    else
        print_ok "Vorhandene Installation gefunden: ${BIN_DIR}/pdf-tool"
    fi
}

update_repository() {
    print_header "Quellcode aktualisieren"
    mkdir -p "${WORK_ROOT}"

    if [[ ! -d "${SOURCE_DIR}/.git" ]]; then
        if [[ -n "${REPO_BRANCH}" ]]; then
            print_info "Lokaler Quellcode fehlt, klone ${REPO_URL} (Branch: ${REPO_BRANCH})"
            git clone --depth 1 --branch "${REPO_BRANCH}" "${REPO_URL}" "${SOURCE_DIR}"
        else
            print_info "Lokaler Quellcode fehlt, klone ${REPO_URL}"
    ...
    if [[ -n "${REPO_BRANCH}" ]]; then
        print_info "Wechsle auf Branch ${REPO_BRANCH}"
        ...
        print_info "Aktualisiere Branch ${current_branch}"
        git -C "${SOURCE_DIR}" fetch --depth 1 origin "${current_branch}"
        git -C "${SOURCE_DIR}" reset --hard "origin/${current_branch}"
    fi

    local updated_commit
    updated_commit="$(git -C "${SOURCE_DIR}" rev-parse --short HEAD)"

    if [[ "${current_commit}" == "${updated_commit}" ]]; then
        print_ok "Repository ist bereits aktuell (${updated_commit})."
    else
        print_ok "Repository aktualisiert: ${current_commit} -> ${updated_commit}"
    fi
}

build_project() {
    print_header "Projekt neu bauen"
    rm -rf "${BUILD_DIR}"
...
install_application() {
    print_header "Installation aktualisieren"
    ...
    print_ok "PDFTool wurde aktualisiert."
}
